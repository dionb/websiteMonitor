...
class Producer(object):
    ...
    def __init__(self, client,
        ...
        if self.async1:
            ...
            self.async1_queue_put_timeout = async_queue_put_timeout
        ...
        if self.async1:
            for idx, m in enumerate(msg):
                try:
                    ...
                    if self.async1_queue_put_timeout == 0:
                        self.queue.put_nowait(item)
                    else:
                        self.queue.put(item, True, self.async1_queue_put_timeout)
                except Full:
    ...
    def __del__(self):
        if self.async1 and not self.stopped:
            self.stop()
